...
export default function ProjectWorkspace() {
  ...
  return <main className="project-shell">
    <aside className="project-sidebar">
      ...
      <nav>{nav.map((item) => <button key={item.id} className={tab === item.id ? "active" : ""} onClick={() => { setTab(item.id); history.replaceState(null, "", `#${item.id}`); }}><i>{item.icon}</i>{item.label}{item.id === "campaigns" && data.campaigns.length > 0 && <em>{data.campaigns.length}</em>}</button>)}</nav>
...
function ModelView({ model, scans, projectNodes, working, approve, rescan }: { model?: Row; scans: Row[]; projectNodes: Row[]; working: boolean; approve: () => void; rescan: () => void }) {
  return <section><PageHead eyebrow="REPOSITORY MODEL" title="Review the detected system" description="Every component must point back to repository evidence. Unknowns remain unknown until runtime validation." action={scans.some((scan) => ["queued", "running", "dispatching"].includes(String(scan.status))) ? "Scan in progress" : "Rescan exact commit"} onAction={rescan} />
    {!model ? <Empty title="Repository scan has not produced a model" text="Reconnect or rescan this GitHub repository. WorldModel will not invent an architecture while the durable scanner is unavailable." action="Open GitHub integration" href="/dashboard?tab=integrations" /> : <><div className="model-summary"><div><span>COMMIT</span><code>{String(model.commit_sha)}</code></div><div><span>CONFIDENCE</span><b>{String(model.confidence)}%</b></div><div><span>COMPONENTS</span><b>{projectNodes.length}</b></div><div><span>STATUS</span><b className={model.status === "approved" ? "good" : "warn"}>{String(model.status)}</b></div></div><div className="model-layout"><div className="model-canvas"><div className="model-grid" />{projectNodes.slice(0, 16).map((node, index) => <article key={String(node.id)} style={{ left: `${12 + (index % 4) * 25}%`, top: `${18 + Math.floor(index / 4) * 24}%` }}><i>{node.kind === "datastore" ? "▤" : node.kind === "journey" ? "↝" : "◇"}</i><span><b>{String(node.name)}</b><small>{String(node.kind)}</small></span><em className={node.confidence === "observed" ? "observed" : "inferred"} /></article>)}</div><aside className="evidence-list"><span>SOURCE EVIDENCE</span>{projectNodes.slice(0, 10).map((node) => <div key={String(node.id)}><b>{String(node.name)}</b><code>{String((node.evidence as string[] | undefined)?.[0] || node.path || "No evidence")}</code><small>{String(node.confidence || "unknown")}</small></div>)}</aside></div>{model.status !== "approved" && <div className="sticky-action"><div><b>Human approval required</b><span>Approval freezes this graph as the environment basis.</span></div><button disabled={working} onClick={approve}>{working ? "Saving…" : "Approve model →"}</button></div>}</>}
  </section>;
}
...
function EnvironmentView({ data, model, environment, working, save }: { data: Snapshot; model?: Row; environment?: Row; working: boolean; save: (payload: Row) => void }) {
  const [backend, setBackend] = useState(String(environment?.backend || "cloudflare_sandbox"));
  ...
  function submit(event: FormEvent) { event.preventDefault(); setManifestError(""); try { save({ modelVersionId: model?.id, backend, manifest: JSON.parse(manifest), approve: true }); } catch { setManifestError("Manifest must be valid JSON before it can be validated."); } }
  return <section><PageHead eyebrow="EXECUTION ENVIRONMENT" title="Review exactly what will run" description="Repository commands remain blocked until this manifest is approved. Hosted sandboxes and GitHub Actions submit the same evidence contract." />
    {environment && <ExecutionReadinessCard projectId={String(data.project.id)} execution={data.execution} />}
    {!model || model.status !== "approved" ? <Empty title="Approve the system model first" text="Environment commands must be pinned to an approved commit and graph version." action="Review model" href="#model" /> : <form className="environment-form" onSubmit={submit}><div className="backend-cards"><button type="button" className={backend === "cloudflare_sandbox" ? "selected" : ""} onClick={() => setBackend("cloudflare_sandbox")}><span>RECOMMENDED</span><b>Cloudflare Sandbox</b><small>Isolated hosted execution with automatic teardown</small></button><button type="button" className={backend === "github_actions" ? "selected" : ""} onClick={() => setBackend("github_actions")}><span>FALLBACK</span><b>GitHub Actions</b><small>Customer-controlled runner for Docker or native dependencies</small></button></div><label><span>WORLDMODEL MANIFEST</span><textarea spellCheck={false} value={manifest} onChange={(event) => setManifest(event.target.value)} /></label>{manifestError && <p className="form-error" role="alert">{manifestError}</p>}<div className="safety-grid"><div><b>Command allowlist</b><span>npm, pnpm, Yarn, and npx only</span></div><div><b>Resource ceiling</b><span>4 CPU · 8 GB · 60 minutes</span></div><div><b>Network</b><span>Deny or package registries only</span></div><div><b>Cleanup</b><span>Required on every terminal state</span></div></div><div className="sticky-action"><div><b>{environment?.status === "approved" ? "Approved environment revision" : "Approval creates an immutable revision"}</b><span>Changing commands later creates a new revision.</span></div><button disabled={working}>{working ? "Validating…" : "Validate & approve →"}</button></div></form>}
  </section>;
}
...
function RunsView({ runs }: { runs: Row[] }) { return <section><PageHead eyebrow="OBSERVED EVIDENCE" title="Run history and replay" description="Only signed evidence from a configured Sandbox or GitHub Actions runner appears here." /><div className="run-table"><header><span>RUN</span><span>SCENARIO</span><span>STATUS</span><span>ERROR RATE</span><span>P95</span><span>JOURNEY</span></header>{runs.map((run) => <article key={String(run.id)}><code>{String(run.id).slice(0, 18)}</code><b>{String(run.scenario)}</b><span className="good">● {String(run.status)}</span><span>{String(run.error_rate)}</span><span>{String(run.latency_ms)}ms</span><span>{String(run.journey_success)}%</span></article>)}</div>{!runs.length && <Empty title="No observed runs" text="Modeled and sample results are intentionally excluded. Approve a campaign after configuring a durable runner." action="Review campaigns" href="#campaigns" />}</section>; }
function ExecutionReadinessCard({ projectId, execution, compact = false, configure }: { projectId: string; execution: ExecutionReadiness; compact?: boolean; configure?: () => void }) {
  return <aside className={`execution-readiness ${execution.ready ? "ready" : "blocked"} ${compact ? "compact" : ""}`} role="status">
    ...
    <div><span>{execution.ready ? "RUNNER READY" : "RUNNER SETUP REQUIRED"}</span><b>{execution.backend === "github_actions" ? "GitHub Actions" : "Cloudflare Sandbox"}</b><p>{execution.message}</p>{!execution.ready && execution.missing.length > 0 && <small>Missing: {execution.missing.join(" · ")}</small>}</div>
