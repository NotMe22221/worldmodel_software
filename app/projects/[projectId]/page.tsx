  ...
  return <main className="project-shell">
    ...
    <section className="project-main">
      ...
      <div className="project-content">
        ...
        {tab === "assistant" && <AssistantView ready={setup.every(Boolean)} working={working} campaigns={data.campaigns} initialConversationId={String(data.conversations[0]?.id || "") || undefined} send={(message, conversationId) => mutate("assistant-message", { message, conversationId })} onCampaigns={() => setTab("campaigns")} />}
        ...
        {tab === "runs" && <LiveRunsView projectId={projectId} projectNodes={projectNodes} campaignRuns={data.campaignRuns} />}
...
function ModelView({ model, scans, projectNodes, working, approve, rescan }: { model?: Row; scans: Row[]; projectNodes: Row[]; working: boolean; approve: () => void; rescan: () => void }) {
  const scanning = scans.some((scan) => ["queued", "running", "dispatching"].includes(String(scan.status)));
  return <section><PageHead eyebrow="REPOSITORY MODEL" title="Review the detected system" description="Every component must point back to repository evidence. Unknowns remain unknown until runtime validation." action={scanning ? "Scan in progress" : "Rescan exact commit"} actionDisabled={scanning} onAction={rescan} />
...
function EnvironmentView({ data, model, environment, working, save }: { data: Snapshot; model?: Row; environment?: Row; working: boolean; save: (payload: Row) => void }) {
  ...
  return <section><PageHead eyebrow="EXECUTION ENVIRONMENT" title="Review exactly what will run" description="Repository commands remain blocked until this manifest and the customer-controlled runner are configured. observeCommand must read WORLDMODEL_EXECUTION_SPEC and write .worldmodel/observed-run.json." />
...
function JourneysView({ journeys, working, create }: { journeys: Row[]; working: boolean; create: (definition: Row) => void }) {
  ...
  return <section><PageHead eyebrow="CRITICAL JOURNEYS" title="Measure outcomes, not only services" description="A run is meaningful only when it proves whether a real user or API workflow succeeded." action="＋ New journey" onAction={() => setShow(true)} />
    {show && <form className="journey-form" onSubmit={submit}><label>Journey name<input name="journeyName" required maxLength={100} value={name} onChange={(event) => setName(event.target.value)} /></label><div><label>Importance<select name="importance" value={importance} onChange={(event) => setImportance(event.target.value)}><option value="critical">Critical</option><option value="high">High</option><option value="normal">Normal</option></select></label><label>Latency SLO (ms)<input name="latencyThresholdMs" type="number" min="50" max="120000" required value={latencyThresholdMs} onChange={(event) => setLatencyThresholdMs(event.target.value)} /></label><label>Allowed errors (%)<input name="allowedErrorRate" type="number" min="0" max="100" step="0.1" required value={allowedErrorRate} onChange={(event) => setAllowedErrorRate(event.target.value)} /></label></div><div><label>Entry path<input name="entryPath" required maxLength={180} value={entryPath} onChange={(event) => setEntryPath(event.target.value)} /></label><label>Success assertion<input name="successAssertion" required maxLength={1000} value={successAssertion} onChange={(event) => setSuccessAssertion(event.target.value)} /></label><label>Journey command<input name="command" required maxLength={240} value={command} onChange={(event) => setCommand(event.target.value)} /></label></div><p>The journey uses the repository&apos;s approved browser test command. The assistant can propose a generated test diff after the environment validates.</p><button disabled={working}>{working ? "Approving…" : "Approve journey →"}</button></form>}
...
function AssistantView({ ready, working, campaigns, initialConversationId, send, onCampaigns }: { ready: boolean; working: boolean; campaigns: Row[]; initialConversationId?: string; send: (message: string, conversationId?: string) => Promise<unknown>; onCampaigns: () => void }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); const result = await send(message, conversationId) as { summary?: string; conversationId?: string }; if (result?.conversationId) setConversationId(result.conversationId); setReply(result?.summary || "Campaign draft created."); }
  return <section className="assistant-page"><PageHead eyebrow="PROJECT AI" title="Plan many tests in one conversation" description="The assistant can inspect approved project evidence and draft a bounded campaign. It cannot execute or write to GitHub without approval." />
    {!ready ? <Empty title="Finish project activation" text="Approve a model, environment, and critical journey before the assistant can produce executable scenarios." action="Review setup" href="#overview" /> : <div className="assistant-layout"><div className="chat-thread"><div className="assistant-message"><i>✦</i><div><b>WorldModel Scenario Copilot</b><p>I can create a campaign of up to 20 traffic, dependency, database, combined-failure, and recovery runs. I will show cost and assumptions before anything executes.</p></div></div>{reply && <div className="assistant-message result"><i>✓</i><div><b>Campaign draft ready</b><p>{reply}</p><button onClick={onCampaigns}>Review campaign →</button></div></div>}<form onSubmit={submit}><textarea aria-label="Reliability campaign request" value={message} onChange={(event) => setMessage(event.target.value)} /><footer><span>✦ Structured scenarios · evidence only</span><button disabled={working || !message.trim()}>{working ? "Planning…" : "Draft campaign ↑"}</button></footer></form></div><aside><span>ASSISTANT PERMISSIONS</span><ul><li><i>✓</i>Read approved model</li><li><i>✓</i>Read journeys and evidence</li><li><i>✓</i>Draft up to 20 scenarios</li><li><i>—</i>Cannot execute without approval</li><li><i>—</i>Cannot publish or merge</li></ul><div><b>{campaigns.length}</b><span>campaign drafts</span></div></aside></div>}
  </section>;
}

function campaignRows(value: unknown) {
  return Array.isArray(value) ? value.filter((item): item is Row => Boolean(item) && typeof item === "object" && !Array.isArray(item)) : [];
}

function CampaignPlanDetails({ plan }: { plan: { scenarios?: Row[]; assumptions?: string[] } }) {
  const scenarios = campaignRows(plan.scenarios);
  const assumptions = Array.isArray(plan.assumptions) ? plan.assumptions.filter((item): item is string => typeof item === "string" && Boolean(item.trim())) : [];
  return <section className="campaign-plan" aria-label="Exact campaign execution plan">
    <header><b>EXACT EXECUTION PLAN</b><span>Review every scenario before approval</span></header>
    <ol>{scenarios.map((scenario, index) => {
      const workload = scenario.workload && typeof scenario.workload === "object" && !Array.isArray(scenario.workload) ? scenario.workload as Row : {};
      const thresholds = scenario.thresholds && typeof scenario.thresholds === "object" && !Array.isArray(scenario.thresholds) ? scenario.thresholds as Row : {};
      const faults = campaignRows(scenario.faults);
      const journeys = Array.isArray(scenario.journeyIds) ? scenario.journeyIds.filter((item): item is string => typeof item === "string") : [];
      return <li key={`${String(scenario.seed || "scenario")}-${index}`}>
        <div><i>{String(index + 1).padStart(2, "0")}</i><span><b>{String(scenario.name || `Scenario ${index + 1}`)}</b><code>{String(scenario.seed || "No seed")}</code></span><strong>{String(scenario.durationSeconds ?? "—")}s</strong></div>
        <p><b>Workload</b> {String(workload.baselineRps ?? "—")} RPS × {String(workload.peakMultiplier ?? "—")} over {String(workload.rampSeconds ?? "—")}s</p>
        <p><b>Faults</b> {faults.length ? faults.map((fault) => `${String(fault.kind || "fault")} on ${String(fault.target || "unknown")} at ${String(fault.startOffsetSeconds ?? "—")}s for ${String(fault.durationSeconds ?? "—")}s`).join(" · ") : "None"}</p>
        <p><b>Hard gates</b> ≤ {String(thresholds.maxErrorRate ?? "—")}% errors · ≤ {String(thresholds.maxP95LatencyMs ?? "—")}ms p95 · ≥ {String(thresholds.minJourneySuccess ?? "—")}% journey success</p>
        <dl className="campaign-bindings"><div><dt>Model</dt><dd><code>{String(scenario.modelVersionId || "—")}</code></dd></div><div><dt>Environment</dt><dd><code>{String(scenario.environmentRevisionId || "—")}</code></dd></div><div><dt>Journeys</dt><dd><code>{journeys.length ? journeys.join(" · ") : "None"}</code></dd></div></dl>
        <small>{journeys.length} {journeys.length === 1 ? "journey" : "journeys"} · cleanup {String(scenario.cleanupPolicy || "—")} · {String(scenario.evidenceMode || "—")} evidence</small>
      </li>;
    })}</ol>
    <div className="campaign-assumptions"><b>ASSUMPTIONS</b>{assumptions.length ? <ul>{assumptions.map((assumption, index) => <li key={`${assumption}-${index}`}>{assumption}</li>)}</ul> : <p>No additional assumptions.</p>}</div>
  </section>;
}
...
function CampaignControlView({ projectId, campaigns, execution, working, configure, approve, cancel }: { projectId: string; campaigns: Row[]; execution: ExecutionReadiness; working: boolean; configure: () => void; approve: (id: string) => void; cancel: (id: string) => void }) {
  return <section><PageHead eyebrow="TEST CAMPAIGNS" title="One approval, many controlled runs" description="Every approved scenario is immutable and uses the same evidence contract." />
    ...
    <div className="campaign-list">{campaigns.map((campaign) => { const plan = parse(campaign.plan_json) as { scenarios?: Row[]; assumptions?: string[] }; const status = String(campaign.status); const active = ["dispatching", "queued", "running"].includes(status); const approvable = ["draft", "dispatch_failed"].includes(status); return <article key={String(campaign.id)}><header><div><span>{status.replaceAll("_", " ").toUpperCase()}</span><h3>{String(campaign.name)}</h3><p>{String(campaign.objective)}</p></div><div><b>{plan.scenarios?.length || 0}</b><small>runs</small></div></header><div className="campaign-facts"><span><b>{String(campaign.concurrency)}</b> concurrent</span><span><b>{String(campaign.estimated_minutes)}m</b> estimated</span><span><b>Observed</b> evidence required</span></div><CampaignPlanDetails plan={plan} />{approvable && <footer><p>{status === "dispatch_failed" ? "The prior dispatch created no observed evidence. It can be safely retried after runner setup." : "Approval authorizes the exact scenarios and assumptions shown above. Repository writes remain separately gated."}</p>{execution.ready ? <button disabled={working} onClick={() => approve(String(campaign.id))}>{working ? "Queuing..." : status === "dispatch_failed" ? "Retry campaign" : "Approve and run campaign"}</button> : <button type="button" onClick={configure}>Configure execution</button>}</footer>}{active && <footer><p>Cancellation requests runner teardown and preserves recorded evidence.</p><button className="danger" disabled={working} onClick={() => cancel(String(campaign.id))}>Cancel campaign</button></footer>}</article>; })}</div>
    {!campaigns.length && <Empty title="No campaigns yet" text="Ask the project AI to turn a reliability objective into an editable multi-run plan." action="Open AI assistant" href="#assistant" />}
  </section>;
}

function LiveRunsView({ projectId, projectNodes, campaignRuns }: { projectId: string; projectNodes: Row[]; campaignRuns: Row[] }) {
  const [selected, setSelected] = useState(String(campaignRuns[0]?.id || ""));
  const selectedRunId = campaignRuns.some((run) => String(run.id) === selected) ? selected : String(campaignRuns[0]?.id || "");
  return <section><PageHead eyebrow="OBSERVED EVIDENCE" title="Live run and replay" description="Ordered events survive refresh. Metrics become observed only after signed runner evidence passes verification." /><div className="run-table"><header><span>RUN</span><span>SCENARIO</span><span>STATUS</span><span>ERROR RATE</span><span>P95</span><span>JOURNEY</span></header>{campaignRuns.map((run) => { const scenario = parse(run.scenario_json) as Row; const runId = String(run.id); return <article key={runId} onClick={() => setSelected(runId)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelected(runId); } }} tabIndex={0} role="button" aria-current={runId === selectedRunId ? "true" : undefined} aria-label={`Open replay for ${String(run.scenario || scenario.name || `scenario ${Number(run.scenario_index) + 1}`)}`}><code>{runId.slice(0, 18)}</code><b>{String(run.scenario || scenario.name || `Scenario ${Number(run.scenario_index) + 1}`)}</b><span className="good">{String(run.status)}</span><span>{run.error_rate == null ? "Pending" : String(run.error_rate)}</span><span>{run.latency_ms == null ? "-" : `${String(run.latency_ms)}ms`}</span><span>{run.journey_success == null ? "-" : `${String(run.journey_success)}%`}</span></article>; })}</div>{selectedRunId && <LiveReplay key={selectedRunId} projectId={projectId} runId={selectedRunId} projectNodes={projectNodes} />}{!campaignRuns.length && <Empty title="No runs yet" text="Modeled and sample results are excluded. Approve a campaign after configuring a durable runner." action="Review campaigns" href="#campaigns" />}</section>;
}
...
function LiveReplay({ projectId, runId, projectNodes }: { projectId: string; runId: string; projectNodes: Row[] }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      polling = true;
      try {
        const response = await fetch(`/api/v1/runs/${encodeURIComponent(runId)}/events?projectId=${encodeURIComponent(projectId)}&after=${after}`, { cache: "no-store" });
        const body = await response.json() as { events?: Row[]; error?: { message?: string } };
        if (!response.ok) throw new Error(body.error?.message || "Replay stream is unavailable");
        if (active && body.events?.length) {
          after = Math.max(after, ...body.events.map((event) => Number(event.sequence || 0)));
          setEvents((current) => {
            const bySequence = new Map(current.map((event) => [Number(event.sequence), event]));
            for (const event of body.events || []) bySequence.set(Number(event.sequence), event);
            return [...bySequence.values()].sort((left, right) => Number(left.sequence) - Number(right.sequence));
          });
        }
        if (active) setStreamError("");
      } catch {
        if (active) setStreamError("Live replay is temporarily unavailable. Retrying automatically.");
      } finally {
        polling = false;
      }
    };
  ...
  const selectedPosition = events.length ? Math.min(selectedIndex ?? events.length - 1, events.length - 1) : 0;
  const visibleEvents = events.slice(0, selectedPosition + 1);
  const state = new Map<string, string>(); for (const event of visibleEvents) { const serviceId = String(event.service_id || ""); if (!serviceId) continue; if (String(event.type).includes("failed")) state.set(serviceId, "failed"); else if (String(event.type).includes("degraded") || event.type === "threshold.breached") state.set(serviceId, "degraded"); else if (String(event.type).includes("recovered")) state.set(serviceId, "healthy"); }
  const selectedEvent = events[selectedPosition]; const selectedPayload = parse(selectedEvent?.payload_json, selectedEvent?.payload || {}) as Row;
  function selectEvent(index: number) { setSelectedIndex(index === events.length - 1 ? null : index); }
  return <div className="twin-replay" aria-live="polite"><section className="twin-canvas"><header><div><span>LIVE DIGITAL TWIN</span><b>{runId}</b></div><strong>{events.length ? selectedIndex === null ? "Following observed stream" : "Replaying observed history" : "Waiting for runner"}</strong></header><div className="twin-nodes">{projectNodes.slice(0, 24).map((node) => { const health = state.get(String(node.id).replace(/^cmp_/, "")) || "unknown"; return <article key={String(node.id)} className={health} tabIndex={0}><i>{health === "healthy" ? "✓" : health === "degraded" ? "!" : health === "failed" ? "×" : "?"}</i><div><b>{String(node.name)}</b><small>{String(node.kind)} · {health}</small></div></article>; })}</div><footer><input aria-label="Replay position" type="range" min="0" max={Math.max(0, events.length - 1)} value={selectedPosition} disabled={!events.length} onChange={(event) => selectEvent(Number(event.target.value))} /><span>{selectedEvent ? new Date(String(selectedEvent.timestamp)).toLocaleTimeString() : "No events"}</span></footer>{streamError && <p className="replay-error" role="status">{streamError}</p>}</section><aside className="twin-inspector"><span>SELECTED EVIDENCE</span><h3>{String(selectedEvent?.type || "No event selected")}</h3><p>{String(selectedEvent?.source || "Runner events will appear here.")}</p>{Object.entries(selectedPayload).slice(0, 8).map(([key, value]) => <div key={key}><b>{key}</b><code>{typeof value === "object" ? JSON.stringify(value) : String(value)}</code></div>)}</aside><section className="live-replay"><header><div><span>ORDERED EVENT REPLAY</span><b>Refresh-safe, selectable timeline</b></div><strong>{events.length} events</strong></header><div>{events.map((event, index) => <article key={String(event.sequence)} role="button" tabIndex={0} aria-current={index === selectedPosition ? "step" : undefined} aria-label={`Replay event ${String(event.sequence)}: ${String(event.type)}`} onClick={() => selectEvent(index)} onKeyDown={(keyboardEvent) => { if (keyboardEvent.key === "Enter" || keyboardEvent.key === " ") { keyboardEvent.preventDefault(); selectEvent(index); } }}><code>{String(event.sequence).padStart(3, "0")}</code><time>{new Date(String(event.timestamp)).toLocaleTimeString()}</time><b>{String(event.type)}</b><span>{String(event.source)}</span></article>)}</div>{!events.length && <p>Waiting for the first durable runner event...</p>}</section></div>;
}
...
function PageHead({ eyebrow, title, description, action, actionDisabled = false, onAction }: { eyebrow: string; title: string; description: string; action?: string; actionDisabled?: boolean; onAction?: () => void }) { return <header className="page-head"><div><span>{eyebrow}</span><h1>{title}</h1><p>{description}</p></div>{action && <button disabled={actionDisabled} onClick={onAction}>{action}</button>}</header>; }
