...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({ error: { code: "runner_token_rejected", message, retriable: false, correlationId: crypto.randomUUID() } }, { status: message.includes("not configured") ? 503 : 403, headers: noStore });
  }
}
