...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!code || !state) return privateRedirect(dashboard(origin, "github_canceled"));
    ...
    return privateRedirect(dashboard(origin, "github_connected"));
  } catch {
    try {
      return privateRedirect(dashboard(await publicRequestOrigin(request), "github_error"));
    } catch {
