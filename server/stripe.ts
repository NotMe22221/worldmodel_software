...
export async function createStripeCheckout(input: {
  ...
  const price =
    input.plan === "starter" ? config.starterPrice : config.proPrice;
  const body = new URLSearchParams({
    mode: "subscription",
    "line_items[0][price]": price,
    "line_items[0][quantity]": "1",
    success_url: `${input.origin}/dashboard?billing=success`,
    cancel_url: `${input.origin}/dashboard?billing=canceled`,
...
export async function createStripePortal(input: {
  ...
  const config = await stripeSecretConfiguration();
  return createStripePortalWithKey({ ...input, secretKey: config.secretKey });
}
