...
export async function exportWorkspaceData(email: string) {
  ...
  return { exportVersion: 1, generatedAt, scope: "workspace", workspace: snapshot.workspace, members: snapshot.members, projects: snapshot.projects, simulationRuns: snapshot.runs, githubInstallations: snapshot.githubInstallations, githubRepositories: snapshot.githubRepositories, subscription, supportCases: supportCases.results, auditEvents: auditEvents.results, launchChecks: snapshot.launchChecks, deletionRequests: deletionRequests.results };
}
