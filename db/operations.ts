...
export async function exportWorkspaceData(email: string) {
  ...
  return { exportVersion: 1, generatedAt, scope: "workspace", workspace: snapshot.workspace, entitlements: snapshot.entitlements, members: snapshot.members, pendingInvitations: snapshot.pendingInvitations, projects: snapshot.projects, simulationRuns: snapshot.runs, repairProposals: snapshot.repairs, githubInstallations: snapshot.githubInstallations, githubRepositories: snapshot.githubRepositories, subscription, supportCases: supportCases.results, auditEvents: auditEvents.results, launchChecks: snapshot.launchChecks, deletionRequests: deletionRequests.results, apiKeys: snapshot.apiKeys, apiUsage: snapshot.apiUsage };
}
