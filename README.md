...
Copy the keys from `.env.example` into the production environment rather than committing secrets. Set `COMPOSIO_API_KEY` and `COMPOSIO_GITHUB_AUTH_CONFIG_ID`, and keep Vercel's **Automatically expose System Environment Variables** project setting enabled. WorldModel then uses `VERCEL_PROJECT_PRODUCTION_URL` as the canonical production origin; set `WORLDMODEL_PUBLIC_ORIGIN` only to choose a different canonical HTTPS domain. Composio hosts GitHub consent and returns to `/api/integrations/composio/github/callback`. Request repository write permissions only when approved draft-PR publication is enabled. The custom GitHub App routes are an advanced fallback, not the primary connection shown to customers. GitHub Actions callbacks exchange a repository/branch-bound OIDC assertion at `/api/v1/runner/token`; evidence is accepted once at `/api/v1/runner/evidence` with an expiring token.
